...
import { addExperiment, queryExperiment, removeExperiment, updateExperiment, queryExperimentVerboseName, queryExperimentListDisplay, queryExperimentDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryCompound, queryCompoundVerboseName} from '@/pages/AutoGenPage/CompoundList/service';import {querySource, querySourceVerboseName} from '@/pages/AutoGenPage/SourceList/service';import {queryDevice, queryDeviceVerboseName} from '@/pages/AutoGenPage/DeviceList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateExperiment(value, current_id);
      hide();
        ...
        hideInSearch: true,
            ...
            hideInForm: true,
                      ...
                      message: 'device为必填项',
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, deviceForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, deviceVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryExperimentDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryExperimentListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [deviceForeignKeyList, setDeviceForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryDevice({all: 1}).then(value => {
                                     setDeviceForeignKeyList(value);
                                });
                                }, []);
                                const [deviceVerboseNameMap, setDeviceVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryDeviceVerboseName().then(value => {
                                    setDeviceVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryExperiment, table_columns, 'experiment-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索experiment" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'experiment-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryExperiment({ ...params, sorter, filter })}
