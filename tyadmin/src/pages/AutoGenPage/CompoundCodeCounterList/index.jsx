...
import { addCompoundCodeCounter, queryCompoundCodeCounter, removeCompoundCodeCounter, updateCompoundCodeCounter, queryCompoundCodeCounterVerboseName, queryCompoundCodeCounterListDisplay, queryCompoundCodeCounterDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addCompoundCodeCounter({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCompoundCodeCounter(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeCompoundCodeCounter(ids);
      hide();
          ...
          width: 100,
                              render: (text, record) => (
                                <>
                                  ...
                                  <Popconfirm
                                    title="您确定要删除compound code counter吗？"
                                    placement="topRight"
                                    onConfirm={() => {
                                      handleRemove([record])
  ...
  useEffect(() => {
    queryCompoundCodeCounterDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCompoundCodeCounterListDisplay().then(value => {
  ...
  return (
    <PageHeaderWrapper>
      <ProTable
        ...
        headerTitle="compound code counter表格"
        actionRef={actionRef}
        rowKey="id"
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCompoundCodeCounter, table_columns, 'compound code counter-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索compound code counter" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'compound code counter-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCompoundCodeCounter({ ...params, sorter, filter })}
