...
import { addDevice, queryDevice, removeDevice, updateDevice, queryDeviceVerboseName, queryDeviceListDisplay, queryDeviceDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateDevice(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryDeviceDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryDeviceListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryDevice, table_columns, 'device-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索device" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'device-select')
                    }
                  }}
                  selectedKeys={[]}
                >
