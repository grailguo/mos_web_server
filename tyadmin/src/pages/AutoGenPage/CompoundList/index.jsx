...
import { addCompound, queryCompound, removeCompound, updateCompound, queryCompoundVerboseName, queryCompoundListDisplay, queryCompoundDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryCompoundTag} from '@/pages/AutoGenPage/CompoundTagList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCompound(value, current_id);
      hide();
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
            ...
            hideInForm: true,
                      ...
                      {
                             title: 'structural_formula',
                             
        hideInSearch: true,
        
                             initialValue: "",
                             dataIndex: 'structural_formula',
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
                              const imageUrl = form.getFieldValue('structural_formula');
                      ...
                      {
                             title: 'safety_data_sheet',
                             
        hideInSearch: true,
        
                             initialValue: "",
                             dataIndex: 'safety_data_sheet',
                        ...
                        render: (text, record) => {
                          ...
                          const downloadUrl = form.getFieldValue('safety_data_sheet');
  ...
  useEffect(() => {
    queryCompoundDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCompoundListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCompound, table_columns, 'compound-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索compound" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'compound-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCompound({ ...params, sorter, filter })}
