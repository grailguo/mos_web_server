...
import { addCompoundTag, queryCompoundTag, removeCompoundTag, updateCompoundTag, queryCompoundTagVerboseName, queryCompoundTagListDisplay, queryCompoundTagDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCompoundTag(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryCompoundTagDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCompoundTagListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCompoundTag, table_columns, 'compound tag-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索compound tag" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'compound tag-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCompoundTag({ ...params, sorter, filter })}
