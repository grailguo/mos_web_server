...
import { addConcUnit, queryConcUnit, removeConcUnit, updateConcUnit, queryConcUnitVerboseName, queryConcUnitListDisplay, queryConcUnitDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateConcUnit(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryConcUnitDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryConcUnitListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryConcUnit, table_columns, 'conc unit-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索conc unit" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'conc unit-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryConcUnit({ ...params, sorter, filter })}
