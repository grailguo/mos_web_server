...
import { addAnalysisType, queryAnalysisType, removeAnalysisType, updateAnalysisType, queryAnalysisTypeVerboseName, queryAnalysisTypeListDisplay, queryAnalysisTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateAnalysisType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryAnalysisTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryAnalysisTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryAnalysisType, table_columns, 'analysis type-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索analysis type" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'analysis type-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryAnalysisType({ ...params, sorter, filter })}
