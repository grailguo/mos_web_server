...
import { addMethodType, queryMethodType, removeMethodType, updateMethodType, queryMethodTypeVerboseName, queryMethodTypeListDisplay, queryMethodTypeDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateMethodType(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryMethodTypeDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryMethodTypeListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryMethodType, table_columns, 'method type-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索method type" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'method type-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryMethodType({ ...params, sorter, filter })}
