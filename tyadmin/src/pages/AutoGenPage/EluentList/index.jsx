...
import { addEluent, queryEluent, removeEluent, updateEluent, queryEluentVerboseName, queryEluentListDisplay, queryEluentDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateEluent(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryEluentDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryEluentListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryEluent, table_columns, 'eluent-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索eluent" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'eluent-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryEluent({ ...params, sorter, filter })}
