...
import { addQuantMethodCodeCounter, queryQuantMethodCodeCounter, removeQuantMethodCodeCounter, updateQuantMethodCodeCounter, queryQuantMethodCodeCounterVerboseName, queryQuantMethodCodeCounterListDisplay, queryQuantMethodCodeCounterDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addQuantMethodCodeCounter({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateQuantMethodCodeCounter(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeQuantMethodCodeCounter(ids);
      hide();
          ...
          width: 100,
                              render: (text, record) => (
                                <>
                                  ...
                                  <Popconfirm
                                    title="您确定要删除quant method code counter吗？"
                                    placement="topRight"
                                    onConfirm={() => {
                                      handleRemove([record])
  ...
  useEffect(() => {
    queryQuantMethodCodeCounterDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryQuantMethodCodeCounterListDisplay().then(value => {
  ...
  return (
    <PageHeaderWrapper>
      <ProTable
        ...
        headerTitle="quant method code counter表格"
        actionRef={actionRef}
        rowKey="id"
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryQuantMethodCodeCounter, table_columns, 'quant method code counter-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索quant method code counter" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'quant method code counter-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryQuantMethodCodeCounter({ ...params, sorter, filter })}
