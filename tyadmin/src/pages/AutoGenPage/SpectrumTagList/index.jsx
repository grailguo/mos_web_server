...
import { addSpectrumTag, querySpectrumTag, removeSpectrumTag, updateSpectrumTag, querySpectrumTagVerboseName, querySpectrumTagListDisplay, querySpectrumTagDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateSpectrumTag(value, current_id);
      hide();
  ...
  useEffect(() => {
    querySpectrumTagDisplayOrder().then(r => {
  ...
  useEffect(() => {
    querySpectrumTagListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, querySpectrumTag, table_columns, 'spectrum tag-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索spectrum tag" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'spectrum tag-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => querySpectrumTag({ ...params, sorter, filter })}
