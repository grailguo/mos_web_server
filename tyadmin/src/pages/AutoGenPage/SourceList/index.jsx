...
import { addSource, querySource, removeSource, updateSource, querySourceVerboseName, querySourceListDisplay, querySourceDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateSource(value, current_id);
      hide();
  ...
  useEffect(() => {
    querySourceDisplayOrder().then(r => {
  ...
  useEffect(() => {
    querySourceListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, querySource, table_columns, 'source-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索source" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'source-select')
                    }
                  }}
                  selectedKeys={[]}
                >
