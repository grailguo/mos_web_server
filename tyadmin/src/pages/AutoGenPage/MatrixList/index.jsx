...
import { addMatrix, queryMatrix, removeMatrix, updateMatrix, queryMatrixVerboseName, queryMatrixListDisplay, queryMatrixDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateMatrix(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryMatrixDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryMatrixListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryMatrix, table_columns, 'matrix-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索matrix" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'matrix-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryMatrix({ ...params, sorter, filter })}
